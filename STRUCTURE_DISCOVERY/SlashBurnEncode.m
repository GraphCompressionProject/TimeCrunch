...
function [model_ordered] = SlashBurnEncode(AOrig, k, outFolder, info, starOption, minSize, graphFile )
...
while niter == 0 || cur_gccsize > k
    ...
    for i=1:size(star_cores, 2)
        ...
        if isinf(MDLcostNC) || isinf(MDLcostST)
            ...
            costGain_notEnc = cost_notEnc / MDLcostST; % cost_notEnc - MDLcostST;
        end
        ...
        elseif starOption == false
            ...
            switch minIdx
                case 1
                    ...
                    costGain_notEnc = cost_notEnc / MDLcostFC;   % cost_notEnc - MDLcostFC;
                    encodeAsFClique( curind, top_gccind, costGain, costGain_notEnc, out_fid, info );
                case 2
                    costGain = MDLcostNC - MDLcostNC;
                    costGain_notEnc = cost_notEnc / MDLcostNC;  % cost_notEnc - MDLcostNC;
                    m = nnz(Asmall);
...
%remove overlapping structures
if numel(model_ordered) > 0
	[model_ordered.nodes] = deal([]);
	for c = 1:numel(model_ordered)
		nodeset = union(model_ordered(c).nodes1, model_ordered(c).nodes2);
		model_ordered(c).nodes = reshape(nodeset,1,numel(nodeset));
	end
	C = cellfun(@char,{model_ordered.nodes},'unif',0);
	[b,i,j] = unique(C);
	model_ordered = model_ordered(sort(i));
end
