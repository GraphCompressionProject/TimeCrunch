...
kept = Ltemporal_greedy(graph_list, TM, errorEnc);

model_lines = open(temp_model_file,'r').readlines();
greedy_fn = '.'.join(temp_model_file.split('.')[:-1]) + '_greedy.tmodel';
outfile_greedy = open(greedy_fn,'w');

for i in kept:
	outfile_greedy.write(model_lines[i]);

outfile_greedy.close();

#get costs, this time with a model
#(total_cost, temporal_model_cost, total_error_cost, model_cost_arr, error_cost_arr, E_arr) = Ltemporal(graph_list, TM, errorEnc);

#print "M_x:\t" + '%.0f' % total_cost + "\t" + '%.0f' % temporal_model_cost + "\t" + '%.0f' % total_error_cost + "\t";
