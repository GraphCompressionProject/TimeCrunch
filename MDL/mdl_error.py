...
def LErrorNaivePrefixLocal(Einc, Eexc):
	#Einc = 1s in error matrix A xor M
	#Eexc = 0s in error matrix A xor M
	if Einc != 0 and Eexc != 0:
		cost = LN(Einc) + Einc*(-log(Einc/float(Einc+Eexc), 2)) + Eexc*(-log(Eexc/float(Einc+Eexc), 2));
	elif Einc != 0:
		cost = LN(Einc);
	elif Eexc != 0:
		cost = LN(Eexc);
