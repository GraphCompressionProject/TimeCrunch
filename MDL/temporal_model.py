...
class TModel :
    ...
    def __init__(self):
        ...
        self.numStrucTypes = len(self.strucTypes) * 5; # oneshot, constant, periodic, bursty, flickering for each structure type
        self.structs = [];        
        self.numStructs = 0;
...
class TStructure :
    ...
    def setTimesteps(self,tsteps):
	self.timesteps = tsteps;

    def getTimesteps(self):
	return self.timesteps;

    def setTemporalType(self,tempType):
	self.temporal_type = tempType;

    def getTemporalType(self):
	return self.temporal_type;
    ...
    def load(line) :
        if line[1:3] == TFullClique.getType() :
            return TFullClique.load(line);
        elif line[1:3] == TNearClique.getType() :
            return TNearClique.load(line);
        elif line[1:4] == TFullOffDiagonal.getType() :
            return TFullOffDiagonal.load(line);
        elif line[1:4] == TNearOffDiagonal.getType() :
            return TNearOffDiagonal.load(line);
        elif line[1:3] == TChain.getType() :
            return TChain.load(line);
        elif line[1:3] == TStar.getType() :
            return TStar.load(line);
        elif line[1:3] == TBiPartiteCore.getType() :
            return TBiPartiteCore.load(line);
        elif line[1:3] == TNearBiPartiteCore.getType() :
            return TNearBiPartiteCore.load(line);
        elif line[1:3] == TCorePeriphery.getType() :
            return TCorePeriphery.load(line);
        elif line[1:3] == TJellyFish.getType() :
            return TJellyFish.load(line);
    load = staticmethod(load)

class TClique(TStructure) :
    nodes = [];
    numNodes = 0;


class TFullClique(TClique) :
    ...
    def load(line) :
        ...
        if line[1:3] != TFullClique.getType() :
            return 0;
        commaparts = line[4:].strip().split(',');
	parts = commaparts[0].split(' ');
	tsteps = [int(x) for x in commaparts[1].strip().split(' ')];
        nodes = [];
        for x in parts :
            if x.find('-') > 0 :
                y = x.strip().split('-');
                ...
                nodes.append(int(x));
	temp = TFullClique(sorted(nodes));
        temp.setTemporalType(line[:1]);
	temp.setTimesteps(tsteps);
	return temp;        
    load = staticmethod(load);


class TNearClique(TClique) :
    numEdges = 0;
    ...
    def load(line) :
        ...
        if line[1:3] != TNearClique.getType() :
            return 0;
        cParts = line[4:].strip().split(',');
        numEdges = int(float(cParts[0].strip()));
        
        sParts = cParts[1].strip().split(' ');
        
	tsteps = [int(x) for x in cParts[2].strip().split(' ')];
        nodes = [];
        for x in sParts :
            if x.find('-') > 0 :
                y = x.strip().split('-');
                ...
                nodes.append(int(x));
	temp = TNearClique(sorted(nodes), numEdges);
	temp.setTemporalType(line[:1]);
        temp.setTimesteps(tsteps);
        return temp;        
    load = staticmethod(load);

class TRectangle(TStructure) :
	lNodeList = [];
	rNodeList = [];
	numNodesLeft = 0;
	numNodesRight = 0;
...
class TFullOffDiagonal(TRectangle) :
    def __init__(self, left, right):
    ...
    def load(line) :
        ...
        if line[:3] != TFullOffDiagonal.getType() :
            return 0;
        ...
        return TFullOffDiagonal(sorted(lNodeList),sorted(rNodeList));
    load = staticmethod(load);


class TNearOffDiagonal(TRectangle) :
    def __init__(self, left, right):
        TRectangle.__init__(self, left, right)
    ...
    def load(line) :
        # "fod [left ids], [right ids]
        if line[:3] != TNearOffDiagonal.getType() :
            return 0;
        parts = line[4:].strip().split(',');
        lParts = parts[0].strip().split(' ');
        lNodeList = [];
        ...
        for x in rParts :
            if x.find('-') > 0 :
                ...
                rNodeList.append(int(x));
        return TNearOffDiagonal(sorted(lNodeList),sorted(rNodeList));
    load = staticmethod(load);



class TChain(TStructure) :
    nodes = [];
    numNodes = 0;
    ...
    def load(line) :
        # "ch 1 2 3 4 ..
        if line[1:3] != TChain.getType() :
            return 0;
        cParts = line[4:].strip().split(',');
	parts = cParts[0].strip().split(' ');
	tsteps = [int(x) for x in cParts[1].strip().split(' ')];
        nodes = [];
        for x in parts :
            if x.find('-') > 0 :
                y = x.strip().split('-');
...
class TStar(TStructure) :
	...
	tsteps = [int(x) for x in parts[2].strip().split(' ')];
        sNodes = [];
        for x in sParts :
            ...
            if x.find('-') > 0 :
                ...
                sNodes.append(int(x));
	temp = TStar(cNodes[0],sorted(sNodes));
	temp.setTemporalType(line[:1]);
	temp.setTimesteps(tsteps);
        return temp;
    load = staticmethod(load);
...
class TBiPartiteCore(TStructure) :
    lNodes = [];
    numNodesLeft = 0;
    rNodes = [];
    numNodesRight = 0;
    ...
    def load(line) :
        ...
        if line[1:3] != TBiPartiteCore.getType() :
            return 0;
        parts = line[4:].strip().split(',');
        lParts = parts[0].strip().split(' ');
        lNodes = [];
	...
	tsteps = [int(x) for x in parts[2].strip().split(' ')];
        for x in rParts :
            if x.find('-') > 0 :
                ...
                rNodes.append(int(x));
	temp = TBiPartiteCore(sorted(lNodes),sorted(rNodes));
	temp.setTemporalType(line[:1]);
	temp.setTimesteps(tsteps);
        return temp;
    load = staticmethod(load);
...
class TNearBiPartiteCore(TStructure) :
    lNodes = [];
    numNodesLeft = 0;
    rNodes = [];
    numNodesRight = 0;
    ...
    def load(line) :
        ...
        if line[1:3] != TNearBiPartiteCore.getType() :
            return 0;
        parts = line[4:].strip().split(',');
        lParts = parts[0].strip().split(' ');
        lNodes = [];
	...
	tsteps = [int(x) for x in parts[2].strip().split(' ')];
        for x in rParts :
            if x.find('-') > 0 :
                ...
                rNodes.append(int(x));
	temp = TNearBiPartiteCore(sorted(lNodes),sorted(rNodes));
	temp.setTemporalType(line[:1]);
	temp.setTimesteps(tsteps);
        return temp;
    load = staticmethod(load);
...
class TCorePeriphery(TStructure) :
    cNodes = [];
    numCores = 0;
    sNodes = [];
    numSpokes = 0;
    ...
    def load(line) :
        # "cp [hubids], [spoke ids]
        if line[:2] != TCorePeriphery.getType() :
            return 0;
        ...
        for x in sParts :
            if x.find('-') > 0 :
                ...
                sNodes.append(int(x));
        return TCorePeriphery(sorted(cNodes),sorted(sNodes));
    load = staticmethod(load);


class TJellyFish(TStructure) :
    cNodes = [];
    numCores = 0;
    sNodes = [[]];
    numSpokes = [];
    ...
    def load(line) :
        ...
        return TJellyFish(sorted(cNodes),sNodes);
    load = staticmethod(load);
