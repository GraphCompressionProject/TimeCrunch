class Graph :
	...
	sg = Graph();
	sg.sgNumNodes = len(nodelist);
	sg.numNodes = self.numNodes;
	
	tmpAdj = [set() for i in range(self.numNodes)];
	for node in nodelist:
		i = node - 1;
		relvEdges = [x for x in self.edges[i] if x+1 in nodelist];
		sg.numEdges += len(relvEdges);
		tmpAdj[i] = relvEdges;

	#sg.edges = [frozenset(x) for x in tmpAdj];
	sg.edges = tmpAdj;
	return sg;
