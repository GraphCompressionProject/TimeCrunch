...
switch subgraph
    case 'fc'
        if E(1) == 0 || E(2) == 0 % no excluded edges
            MDLcost = LN( n_sub ) + l2cnk(N_tot, n_sub);
        else
            MDLcost = LN( n_sub ) + l2cnk(N_tot, n_sub) + Lnu_opt( E );
        end
    case 'nc'
    ...
    case 'st'
        if E(1) == 0 || E(2) == 0 %if sum(sum(E)) == 0
            MDLcost = LN( n_sub-1 ) + log2( N_tot ) + l2cnk( N_tot-1, n_sub-1 );
        else
            MDLcost = LN( n_sub-1 ) + log2( N_tot ) + ...
                l2cnk( N_tot-1, n_sub-1 ) + Lnu_opt( E );
        end
    case 'ch'
        x = 0:(n_sub-1);
    ...
    case 'bc'
        ...
        l = n_sub2;
        if E(1) == 0 || E(2) == 0 %if sum(sum(E)) == 0
            MDLcost = LN(k) + LN(l) + l2cnk(N_tot, k) + l2cnk(N_tot-k, l);
        else
            MDLcost = LN(k) + LN(l) + l2cnk(N_tot, k) + ...
                l2cnk(N_tot-k, l) + Lnu_opt( E );
        end
    case 'nb'
        k = n_sub;
        l = n_sub2;
...
%% Alternative error per structure: Naive Data-to-Model
    function [ c_err ] = Lnd( E )
        Einc = nnz(E);
        c_err = LN( Einc ) + l2cnk(N_tot^2, Einc);
    end
